        ...
        if filename.endswith('.json') and not filename.startswith(output_file_base):
            try:
                with open(file_path, 'r', encoding='utf-8') as file:
                    data = json.load(file)
                    if any(search_string in json.dumps(value) for value in data.values()):
...
# Fields to extract (easy to modify here)
fields_to_extract = ['tenant_name', 'appid_name', 'srcport', 'totalbytes', 'totalpackets', 'dstip', 'dstport', 'login_result', 'srcip_host', 'srcip_username']

# Getting user inputs
search_string = input("Enter the exact string you want to search for in the folder: ")
folder_path = input("Please input the file path for a folder containing JSON files (Ensure there is an embedded folder named Output): ")
