...
def extract_data_from_json(filename, fields):
    extracted_data = {}
    try:
...
def aggregate_and_export_data(folder_path, found_files, fields, output_file):
    aggregated_data = []
    for filename in found_files:
        file_path = os.path.join(folder_path, filename)
        file_data = extract_data_from_json(file_path, fields)
        if file_data:
